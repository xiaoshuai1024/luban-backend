...
class QuotaServiceTest {
    ...
    @BeforeEach
    void setUp() {
        service = new QuotaService(subscriptionRepository, planRepository);
    }
    ...
    void checkAndIncrement_zero_quota_unlimited() {
        ...
        when(planRepository.getByCode("growth")).thenReturn(Optional.of(planOf("growth", 0, 0, 0)));
    ...
    void checkAndIncrement_under_quota_passes() {
        when(subscriptionRepository.findByUserId("u1")).thenReturn(trialingAgg("u1"));
        when(planRepository.getByCode("growth")).thenReturn(Optional.of(planOf("growth", 100, 50, 1000)));
